  ...
  public void execute(final File target) throws IOException {
    ...
    for (final File file : files) {
      try (final FileInputStream inputStream = new FileInputStream(file)) {
        ...
        generator.add(inputStream, FileUtilities.getWithoutExtension(file).getName());
      } catch (final FileNotFoundException exception) {
        String message = "Couldn't find file: " + file;
        this.output.error(message, exception);
        throwables.add(new IOException(message, exception));
      } catch (final SecurityException exception) {
        String message = "Couldn't read file: " + file + ", because " + exception.getMessage();
        this.output.error(message, exception);
        throwables.add(new IOException(message, exception));
      } catch (final JssdParserException exception) {
        String message = exception.getMessage() + ", in file: " + file;
        this.output.error(message, exception);
        throwables.add(new IOException(message, exception));
      } catch (final CreationException exception) {
        throwables.add(exception);
        String message = "Couldn't create bean description from file: " + file + ", because: " + exception.getMessage();
        this.output.error(message, exception);
