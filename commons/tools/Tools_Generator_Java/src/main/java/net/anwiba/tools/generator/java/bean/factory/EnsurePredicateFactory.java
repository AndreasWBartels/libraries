...
public class EnsurePredicateFactory extends AbstractSourceFactory {
  ...
  private final JClass conditionsClass;
  ...
  public JStatement ensureArgumentNotNull(final JVar param) {
    ...
    return this.ensureClass.staticInvoke("ensureThatArgument") //$NON-NLS-1$
        .arg(param)
        .arg(this.conditionsClass.staticInvoke("notNull")); //$NON-NLS-1$
  }

  public JStatement ensureNotNull(final JVar param) {
    ensureThatArgument(param, notNull());
    return this.ensureClass.staticInvoke("ensureThat") //$NON-NLS-1$
        .arg(param)
        .arg(this.conditionsClass.staticInvoke("notNull")); //$NON-NLS-1$
  }
