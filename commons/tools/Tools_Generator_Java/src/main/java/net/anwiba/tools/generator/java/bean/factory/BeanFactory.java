  ...
  public void createGetter(
    ...
    if (getter.isEnabled()) {
      if (getter.isNamedValueGetterEnabled()) {
        final NamedValueProvider namedValueProvider = new NamedValueProvider(Builders
            .type(member.type().generics()[1])
            .build(), member.name(), false, false, new ArrayList<Annotation>(), MessageFormat.format(
            "{0}Names", getter.name()), //$NON-NLS-1$
    ...
    if (configuration.isMutable() && setter.isEnabled()) {
      if (isInstanceOfMap(field.type())) {
        if (setter.isMultiValue()) {
          final List<Annotation> annotations = setter.isSingleValue() ? new ArrayList<Annotation>() : setter
              .annotations();
          this.setterFactory.create(instance, //
              field, //
              setter.name(), //
              member.isNullable(), //
              annotations, //
