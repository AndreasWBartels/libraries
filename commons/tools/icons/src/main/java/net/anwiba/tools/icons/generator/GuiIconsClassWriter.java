...
public class GuiIconsClassWriter implements Closeable {
  ...
  public void write(final Map<String, String> folders, final Map<String, IconResource> configuration)
    ...
    this.writer.append(
        MessageFormat.format("  private final static String _FOLDER = \"{0}\";\n", //$NON-NLS-1$
            getPath(this.targetClass.getPackage())));
    for (final Map.Entry<String, String> folder : folders.entrySet()) {
      this.writer.append(
          MessageFormat.format("  private final static String _{0} = _FOLDER+\"{1}/\";\n", //$NON-NLS-1$
              folder.getValue(),
              folder.getKey()));
    }
    this.writer.append("\n"); //$NON-NLS-1$
    ...
    for (final String name : names) {
      ...
      if (Arrays.equals(
        ...
        if (iconResource.isDecorator()) {
          this.writer.append(
              MessageFormat.format(
                  "  public final static {0} {1} = GuiIcon.of(_h,//\n" //$NON-NLS-1$
                      + "      IconSize.of({3},\"{4}/{2}\",{5}),\n" //$NON-NLS-1$
                      + "      IconSize.of({3},\"{6}/{2}\",{7}),\n" //$NON-NLS-1$
                      + "      IconSize.of({3},\"{8}/{2}\",{9}),\n" //$NON-NLS-1$
                      + "      {10});\n", //$NON-NLS-1$
  ...
  public void initialize() throws IOException {
    ...
    this.writer.append("import java.net.URL;\n"); //$NON-NLS-1$
    this.writer.append("import java.util.function.Function;"); //$NON-NLS-1$
    this.writer.append("\n"); //$NON-NLS-1$
    this.writer.append(MessageFormat.format("public class {0} '{\n", this.targetClass.getName())); //$NON-NLS-1$
    this.writer.append("\n"); //$NON-NLS-1$
    this.writer.append("  private static class Helper implements Function<String, URL> {\n");
    this.writer.append("    public URL apply(String path) {\n");
    this.writer.append("      return getClass().getResource(path);\n");
    this.writer.append("    }\n");
    this.writer.append("  }\n");
    this.writer.append("\n"); //$NON-NLS-1$
    this.writer.append("  private static Helper _h = new Helper();\n");
    this.writer.append("\n"); //$NON-NLS-1$
  ...
  private Object getPath(final String pakkage) {
    return pakkage == null || pakkage.isBlank()
        ? "icons/"
        : pakkage.replaceAll("\\.", "/") + "/icons/";
  }

  private String getParentPath(final String folder) {
    return folder == null ? "_FOLDER" : "_" + folder;
  }
