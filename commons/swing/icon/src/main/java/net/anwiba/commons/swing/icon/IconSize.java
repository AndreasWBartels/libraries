...
public class IconSize implements IIconSize {
  ...
  public static IIconSize of(final String parent, final String path, final int size) {
    final String relativePath = path.startsWith("/") ? path : "/" + path; //$NON-NLS-1$//$NON-NLS-2$
    if (parent == null) {
      return of(relativePath, size);
    }
    final String parentPath = parent.startsWith("/") ? parent.substring(1, parent.length()) : parent; //$NON-NLS-1$
    return of(
        parentPath.endsWith("/") ? parentPath.substring(0, parentPath.length() - 1) + relativePath //$NON-NLS-1$
            : parentPath + relativePath,
        size);
  }

  public static IIconSize of(final String path, final int size) {
