...
public class InternalExceptionHandler implements IExceptionHandler {
  ...
  public InternalExceptionHandler() {
    // Necessary for instantiation from the event dispatch thread
  }

  @Override
  public void handle(final Throwable exception) {
    CentralExceptionHandling.getInstance()
        .handle(new RuntimeException("Uncaught exception on event dispatch thread", exception));
  }
}