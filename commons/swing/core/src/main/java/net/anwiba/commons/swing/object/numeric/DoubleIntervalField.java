...
public class DoubleIntervalField implements IObjectField<DoubleInterval> {
    ...
    public IValidationResult validate(final String string) {
      try {
        ...
        return IValidationResult.inValid(
            MessageFormat
                .format(ObjectFieldMessages.ValueIsOutOfBounderies_0_1,
        ...
        if (!DoubleIntervalField.this.minimumValidationModel.get().isValid()) {
          validationModel.set(
              IValidationResult.inValid(
                  ObjectFieldMessages.IllegalMinimumValue
                      + DoubleIntervalField.this.minimumValidationModel.get().getMessage()));
          return;
        }
        if (!DoubleIntervalField.this.maximumValidationModel.get().isValid()) {
          validationModel.set(
              IValidationResult.inValid(
                  ObjectFieldMessages.IllegalMaximumValue
                      + DoubleIntervalField.this.maximumValidationModel.get().getMessage()));
          return;
        }
  ...
  private IObjectField<Double> createDoubleField(
    ...
    If.isTrue(Double.isFinite(this.stepSize))
        .execute(
