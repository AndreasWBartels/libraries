...
public class WorkQueue implements IWorkQueue {
  ...
  public static final class ProcessScheduledFuture<V> implements RunnableScheduledFuture<V> {
    ...
    public void run() {
      final ICancelerListener listner = new ICancelerListener() {
        ...
        public void canceled() {
          if (ProcessScheduledFuture.this.future.isCancelled()) {
            return;
          }
          ProcessScheduledFuture.this.future.cancel(true);
        }
      };
      try {
    ...
    public boolean isCancelled() {
      return this.runnable.isCancelled() && this.future.isCancelled();
    }
  ...
  public static IWorkQueue create(
    ...
    final ScheduledThreadPoolExecutor executor = new ScheduledThreadPoolExecutor(
      ...
      public ScheduledFuture<?> schedule(
          ...
          final long delay,
          final TimeUnit unit) {
        final ProcessScheduledFuture<?> future = (ProcessScheduledFuture<?>) super.schedule(
            command,
            delay,
            unit);
        handler
  ...
  @Override
  public IProcessIdentfier execute(final IRunnable runnable) throws IllegalStateException {
    try {
