...
class AllOfCondition<T> extends AbstractCondition<T> {

  private final List<ICondition<? super T>> conditions;

  AllOfCondition(final List<ICondition<? super T>> conditions) {
  ...
  public String toText() {
    return MessageFormat.format("all of {0}", //$NON-NLS-1$
        ObjectUtilities.toString(this.conditions));
  }
}
