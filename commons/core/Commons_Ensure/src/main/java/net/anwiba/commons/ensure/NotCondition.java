...
class NotCondition<T> extends AbstractCondition<T> {
  ...
  NotCondition(final ICondition<T> condition) {
    this.condition = condition;
  }

  @Override
  public boolean accept(final T value) {
    return !this.condition.accept(value);
  }

  @Override
  public String toText() {
    return MessageFormat.format("not {0}", //$NON-NLS-1$
        ObjectUtilities.toString(this.condition));
  }
}
