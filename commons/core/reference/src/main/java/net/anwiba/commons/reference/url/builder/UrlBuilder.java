...
public class UrlBuilder {
  ...
  public UrlBuilder(final List<String> scheme, final IHost host, final List<String> path) {
    this.scheme.addAll(scheme);
    this.hostName = Optional.of(host).convert(a -> a.getName()).get();
    this.port = Optional.of(host).convert(a -> Integer.valueOf(a.getPort())).get();
    this.username = null;
    this.password = null;
  ...
  public UrlBuilder(final URL url) {
    ...
    this.username = Optional.of(url.getUserInfo()).convert(u -> u.split(":")[0]).get();
    this.password = Optional.of(url.getUserInfo()).convert(u -> u.split(":")[1]).get();
...
//    this.path.addAll(url.getPath());
  ...
  public UrlBuilder(final URI url) {
    this.scheme.addAll(Arrays.asList(url.getScheme().split(":")));
    this.username = Optional.of(url.getUserInfo()).convert(u -> u.split(":")[0]).get();
    this.password = Optional.of(url.getUserInfo()).convert(u -> u.split(":")[1]).get();
    this.hostName = url.getHost();
    this.port = url.getPort();
...
//    this.path.addAll(url.getPath());
  ...
  public UrlBuilder addQueryParameter(final String name, final String value) {
    addQueryParameter(Parameter.of(name, value));
    return this;
  }

  public UrlBuilder setScheme(String...  scheme) {
    this.scheme.clear();
    this.scheme.addAll(Arrays.asList(scheme));
    return this;
  }
