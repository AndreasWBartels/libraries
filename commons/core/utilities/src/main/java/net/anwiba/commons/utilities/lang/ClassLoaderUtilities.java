...
public class ClassLoaderUtilities {
        ...
        if (!uri.isAbsolute()) {
          try {
            final File file = new File(
                parent == null
                    ? new File(System.getProperty("user.dir")) //$NON-NLS-1$
                    : new File(new URI(parent)),
                string);
  ...
  public static URI[] getClassPathUris(final ClassLoader classLoader) {
    ...
    if (!(classLoader instanceof URLClassLoader)) {
      try {
        return Streams
            .of(classLoader.getResources("META-INF/MANIFEST.MF"))
            .convert(u -> convert(u))
            .notNull()
            .asArray(URI[]::new);
      } catch (IOException e) {
  ...
  private static URI convert(final URL u) {
    try {
      ...
      if (string
          .substring(
              length - 4 < 0
                  ...
                  : length - 4,
              length)
          .equalsIgnoreCase("." + IFileExtensions.JAR)) { //$NON-NLS-1$
