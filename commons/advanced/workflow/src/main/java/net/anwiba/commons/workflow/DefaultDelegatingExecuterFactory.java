...
public class DefaultDelegatingExecuterFactory<T> implements IExecuterFactory<T> {

  @Override
  public IExecutable<T> create(final IExecutable<T> executable) throws RuntimeException {
    return new IExecutable<T>() {

      @Override
      public void execute(final IObjectReceiver<IState<T>> receiver) throws RuntimeException {
        executable.execute(receiver);
      }
    };
  }
