...
public final class PoolWorker extends Thread {
  ...
  private void execute(final IRunnable runnable) {
    try {
      ...
      this.logger.log(ILevel.ERROR, MessageFormat.format("{0}: Error on Runnable[{1} {2}].", //$NON-NLS-1$
              getName(),
              runnable.getClass().getName(),
              runnable.getIdentifier()), e);
    } finally {
    ...
    synchronized (this.queue) {
      while (this.queue.isEmpty()) {
        try {
          ...
          // this.logger.log(ILevel.DEBUG, getName(), exception);
        }
      }
    ...
    if (this.logger.isLoggable(ILevel.DEBUG)) {
      this.logger
          .log(
              ...
              MessageFormat
                  .format(
                      "{0}: dequeued Process {2} Runnable[{1}]. Current size of queue: {3}", getName(), runnable.getClass().getName(), runnable.getIdentifier(), String.valueOf(queueSize))); //$NON-NLS-1$
    }
    return runnable;
  }
