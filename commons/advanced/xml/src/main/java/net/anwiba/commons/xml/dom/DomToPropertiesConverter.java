...
public class DomToPropertiesConverter implements IDomToObjectConverter<IProperties> {
  ...
  @Override
  public IProperties convert(final Element element) throws DomConverterException {
    if (element == null) {
      return Properties.empty();
    }
    final PropertiesBuilder builder = Properties.builder();
    for (final Element propertyElement : elements(element, "property")) { //$NON-NLS-1$
      builder.put(this.domToPropertyConverter.convert(propertyElement));
    }
