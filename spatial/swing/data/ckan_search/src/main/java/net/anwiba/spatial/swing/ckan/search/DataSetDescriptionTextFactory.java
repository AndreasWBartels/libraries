...
public final class DataSetDescriptionTextFactory {
  ...
  public String create(final Dataset dataset) {
    ...
    if (dataset.getTags() != null && dataset.getTags().length > 0) {
      ...
      text
          .append(
              StringUtilities
                  .concat(", ", Streams.of(dataset.getTags()).convert(g -> CkanUtilities.toString(g)).asList())); //$NON-NLS-1$
      text.append("</p>"); //$NON-NLS-1$
    }
    ...
    if (organization != null) {
      ...
      if (organization.getHomepage() != null) {
        text.append("<a href=\"" + organization.getHomepage() + "\">" + CkanUtilities.toString(organization) + "</a>"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
      } else {
  ...
  private String converToUrl(final String email) {
    return email.trim().toLowerCase().startsWith("mailto:") //$NON-NLS-1$
        ? email.trim()
        : "mailto:" + email.trim(); //$NON-NLS-1$
  }
}
