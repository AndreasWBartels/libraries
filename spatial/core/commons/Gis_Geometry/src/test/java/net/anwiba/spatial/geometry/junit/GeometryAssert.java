...
public class GeometryAssert extends CoordinateSequenceAssert {
  ...
  private static void assertEquals(final String message, final IGeometry expected, final IGeometry actual) {
    ...
    switch (expected.getGeometryType()) {
      ...
      case UNKNOWN: {
        for (int i = 0; i < expected.getNumberOfGeometries(); i++) {
          if (!expected.getGeometryType().equals(actual.getGeometryType())) {
            fail(message
                + "base geometry " + i + ", geometryType differed, expected.geometryType=" + expected.getGeometryType() + " actual.geometryType=" //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
                + actual.getGeometryType());
            return;
          }
          final IBaseGeometry expectedBaseGeometry = expected.getGeometryN(i);
    ...
    switch (expected.getGeometryType()) {
      ...
      case POLYGON: {
        ...
        if (expectedPolygon.getNumberOfInnerRings() != actualPolygon.getNumberOfInnerRings()) {
          fail(message
              + ", number off inner rings differed, expected.number=" + expectedPolygon.getNumberOfInnerRings() + " actual.number=" //$NON-NLS-1$ //$NON-NLS-2$
